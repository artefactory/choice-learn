...
class ConditionalMNL(ChoiceModel):
    ...
    def _instantiate(
        ...
        """Instantiate the model from self.coefficients and a dataset.
        ...
        for feat_tuple in shared_features_names:
            ...
            for feat in feat_tuple:
                if feat in self.coefficients.keys():
                    if self.coefficients[feat] == "constant":
                        weight = tf.Variable(
                            tf.random_normal_initializer(0.0, 0.02, seed=42)(shape=(1, 1)),
                            name=feat,
                        )
                    elif self.coefficients[feat] == "item":
                        weight = tf.Variable(
                    ...
                    elif self.coefficients[feat] == "item-full":
                        weight = tf.Variable(
                        ...
                        raise NotImplementedError(
                            f"Param {self.coefficients[feat]} not implemented"
                        )
                    weights.append(weight)
                    ...
                    if self.coefficients[feat] == "constant":
                        ...
                        for i, s_tuple in enumerate(shared_features_names):
                            for j, s_feat in enumerate(s_tuple):
                                if s_feat == self.coefficients[feat]:
        ...
        if "intercept" in self.coefficients.keys():
            if self.coefficients["intercept"] == "constant":
                weight = tf.Variable(
                    tf.random_normal_initializer(0.0, 0.02, seed=42)(shape=(1, 1)), name="intercept"
                )
            elif self.coefficients["intercept"] == "item":
                weight = tf.Variable(
                    tf.random_normal_initializer(0.0, 0.02, seed=42)(shape=(1, n_items - 1)),
                    name="intercept",
                )
            elif self.coefficients["intercept"] == "item-full":
                weight = tf.Variable(
                    tf.random_normal_initializer(0.0, 0.02, seed=42)(shape=(1, n_items)),
                    name="intercept",
                )
                ...
                # Is supposed to be ?
                raise NotImplementedError(f"Param {self.coefficients['intercept']} not implemented")
            weights.append(weight)
        else:

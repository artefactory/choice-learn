"""Classes to handle datasets with baskets of product."""
...
class TripDataset:
    ...
    def __init__(
        self, trips: list[Trip], available_items: np.ndarray, prices: Union[np.ndarray, None] = None
    ) -> None:
    ...
    def iter_batch(
        ...
        ------
        tuple[np.ndarray]
            For each item in the batch: item, basket, store, week, prices, available items
            Length must 6
        """
        # Get trip indexes
        num_trips = len(self)

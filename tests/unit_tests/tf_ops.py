...
def test_softmax():
    ...
    """
    softmax_probabilities = softmax_with_availabilities(items_logit_by_choice=logits, available_items_by_choice=availabilities).numpy()
...
def test_CustomCategoricalCrossEntropy():
    loss = CustomCategoricalCrossEntropy()
    assert loss([[0., 1.]], [[0., 1.]]) == 0.0
    assert loss([[0., 1.]], [[1., 0.]]) > 20.0

    loss = CustomCategoricalCrossEntropy(sparse=True)
    assert loss([1], [[0., 1.]]) == 0.0
    assert loss([1], [[1., 0.]]) > 20.0


    loss = CustomCategoricalCrossEntropy(sparse=True, from_logits=True)
    assert loss([1], [[21., 0.]]) > 20.0